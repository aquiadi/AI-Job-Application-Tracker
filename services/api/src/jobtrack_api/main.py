...
without a web server, and every Google Cloud dependency sits behind an interface so
the same domain code runs against the local stack.
"""
...
app = FastAPI(
    ...
    version=__version__,
    lifespan=lifespan,
    # The web app generates its TypeScript client from this document, so the
    # operation ids have to be stable and readable rather than auto-derived.
